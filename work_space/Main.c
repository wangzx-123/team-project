...
#define find_max(num,num_length) do{\
	int max=DISK_NOT_USE;           \
	int max_id=0;                   \
...
#define find_exit(num,num_length,key) do{ \
 	for(int i=1;i<num_length;i++){        \
...
int select_disk(int *object_replica,int replica_current_point){
	if(find_exit(object_replica,(REP_NUM+1),find_max(disk_size_left,(N+1)))){
		object_replica[replica_current_point]=find_max(disk_size_left,(N+1));
	}
	else {
